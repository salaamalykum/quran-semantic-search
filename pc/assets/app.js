...
function buildSidebar() {
  ...
  for (let s = 1; s <= 114; s++) {
    ...
    btn.className = 'sbtn';
    btn.textContent = `第 ${s} 章：${(window.SURA_NAMES && window.SURA_NAMES[String(s)]) || ''}`;
...
function bindUI() {
  ...
  // Clear existing if any (re-binding safety)
...
function init() {
  // Ensure default theme if not set
  ...
  bindUI();
  loadAll().then(() => {
    // Check URL hash for sura navigation (e.g. #1)
    const hash = window.location.hash.replace('#', '');
    if (hash && !isNaN(hash)) {
      const s = parseInt(hash, 10);
      if (s >= 1 && s <= 114) {
        state.activeSura = s;
        // Highlight sidebar button
        const btn = $$('.sbtn')[s - 1]; // 0-indexed
